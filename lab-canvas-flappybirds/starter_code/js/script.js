window.onload = function () {
  ...
  const beardPos = { x: canvas.width / 2, y: canvas.height / 2 };
  const player1 = new Player(beardPos.x, beardPos.y);
  ...
  startGame = () => {
    clear();
    //1.Option - background image.
    // ctx.drawImage(bgImage, 0, 0, canvas.width, canvas.height - 70);

    //2.Option - background animation image.
    move();
    draw();
    //draw obstacles
    if (obstacles.length > 0) {
      for (let i = 0; i < obstacles.length; i++) {
        ...
        if (Math.round(obstacles[i].x + obstacles[i].width) == 0) {
          score += 5;
        }
      }
    }
